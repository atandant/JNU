...
int process_spawn(const char *path, char *const *argv, int *pid_out) {
  ...
  if (err) {
    /* pr_info("process_spawn: open_exec_image failed for path '%s' (err=%d)\n",
            path, err); */
    goto fail_space;
  }
  ...
  if (!err) {
    err = elf64_setup_initial_stack(child->space, &child->user_stack);
  }
  if (vfs_ino) {
    vfs_close(vfs_ino);
  }
  ...
  if (err) {
    /* pr_info("process_spawn: sched_create_user_task failed (err=%d)\n", err); */
    goto fail_space;
  }
