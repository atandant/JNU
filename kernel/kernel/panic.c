...
{
	uint64_t v; __asm__ __volatile__ ("mov %%cr0, %0" : "=r"(v));
	return v;
}
static uint64_t read_cr3(void)
{
	uint64_t v; __asm__ __volatile__ ("mov %%cr3, %0" : "=r"(v));
	return v;
}
static uint64_t read_cr4(void)
{
	uint64_t v; __asm__ __volatile__ ("mov %%cr4, %0" : "=r"(v));
	return v;
}
...
static void emit_regs(const struct cpu_state *st, uint64_t cr2)
{
	...
	emit_fmt("RAX=0x%016lx   RBX=0x%016lx\n",
		 (unsigned long)st->rax, (unsigned long)st->rbx);
	emit_fmt("RCX=0x%016lx   RDX=0x%016lx\n",
		 (unsigned long)st->rcx, (unsigned long)st->rdx);
	emit_fmt("RSI=0x%016lx   RDI=0x%016lx\n",
		 (unsigned long)st->rsi, (unsigned long)st->rdi);
	emit_fmt("RBP=0x%016lx   R8 =0x%016lx\n",
		 (unsigned long)st->rbp, (unsigned long)st->r8);
	emit_fmt("R9 =0x%016lx   R10=0x%016lx\n",
		 (unsigned long)st->r9,  (unsigned long)st->r10);
	emit_fmt("R11=0x%016lx   R12=0x%016lx\n",
		 (unsigned long)st->r11, (unsigned long)st->r12);
	emit_fmt("R13=0x%016lx   R14=0x%016lx\n",
		 (unsigned long)st->r13, (unsigned long)st->r14);
	emit_fmt("R15=0x%016lx\n\n", (unsigned long)st->r15);

	emit_fmt("CR0=0x%016lx   CR2=0x%016lx\n",
		 (unsigned long)read_cr0(), (unsigned long)cr2);
	emit_fmt("CR3=0x%016lx   CR4=0x%016lx\n\n",
		 (unsigned long)read_cr3(), (unsigned long)read_cr4());
}
