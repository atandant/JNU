...
__used
__section(".limine_requests_start") static volatile LIMINE_REQUESTS_START_MARKER
...
static struct limine_file *find_initramfs_module(void) {
  ...
  for (uint64_t i = 0; i < module_request.response->module_count; i++) {
    ...
    if (file && file->cmdline && strcmp(file->cmdline, "initramfs") == 0) {
      return file;
    }
  }
...
static void load_userspace_probe(void) {
  ...
  if (err) {
    panic("userspace: failed to load %s from initramfs (err=%d)", init_path,
          err);
  }
  ...
  if (err) {
    panic("userspace: failed to load /bin/hello from initramfs (err=%d)", err);
  }
  ...
  if (err) {
    pr_warn("userspace: /hello not executable from MINIX yet (err=%d)\n", err);
  } else {
    pr_info("userspace: /hello MINIX ELF64 entry=0x%lx range=0x%lx..0x%lx\n",
            (unsigned long)minix_info.entry, (unsigned long)minix_info.low,
  ...
  for (int sec = 0; sec < 8; sec++) {
    ...
    for (int row = 0; row < 32; row++) {
      ...
      pr_info("dump: %04x: "
              ...
              "%02x %02x %02x %02x "
              "%02x %02x %02x %02x\n",
              (unsigned)(sec * 512 + off), buf[off + 0], buf[off + 1],
              buf[off + 2], buf[off + 3], buf[off + 4], buf[off + 5],
              buf[off + 6], buf[off + 7], buf[off + 8], buf[off + 9],
              buf[off + 10], buf[off + 11], buf[off + 12], buf[off + 13],
