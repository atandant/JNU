...
static bool ata_identify(struct ata_channel *ch, uint8_t sel,
                         uint16_t *id_buf) {
...
static int ata_bdev_read(struct block_device *bdev, uint64_t lba, size_t count,
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    ata_select_drive(
        ch, (uint8_t)(drv->drive_sel | (uint8_t)((sector >> 24) & 0x0Fu)));

    outb((uint16_t)(ch->io_base + ATA_REG_SECCOUNT), 1);
    outb((uint16_t)(ch->io_base + ATA_REG_LBA_LO), (uint8_t)(sector & 0xFFu));
    ...
    insw((uint16_t)(ch->io_base + ATA_REG_DATA), p, ATA_SECTOR_SIZE / 2);
    p += ATA_SECTOR_SIZE;
  }
...
void ata_init(void) {
  ...
  for (int ch_idx = 0; ch_idx < 2; ch_idx++) {
    ...
    for (int drv_idx = 0; drv_idx < 2; drv_idx++) {
      ...
      d->sectors = (uint64_t)id_buf[60] | ((uint64_t)id_buf[61] << 16);
