#!/usr/bin/env python3
# scripts/gen-font.py — generate kernel/drivers/font_data.h.
#
# Produces a 256-glyph 8x16 monospace font as a C header. The header
# contains a single `static const uint8_t font_8x16[256][16]` table:
# each row is a byte where bit 7 (0x80) is the leftmost pixel, bit 0
# is the rightmost.
#
...
def emit_header(out_path: Path):
    parts = []
    parts.append(
        "/*\n"
        " * kernel/drivers/font_data.h — generated by scripts/gen-font.py.\n"
        " *\n"
        " * 256 glyphs * 16 rows. Each byte's high bit is the leftmost\n"
        " * pixel.\n"
        " *\n"
        " * Source font: Built-in VGA 8x16\n"
        " *\n"
        " * Do not edit by hand; rerun `make font` (or build will\n"
        " * regenerate it automatically as a Make dependency).\n"
        " *\n"
        " * Copyright (c) 2026 The JNU Authors.\n"
        " * SPDX-License-Identifier: GPL-2.0-only\n"
        " */\n\n"
    ...
    for i, rows in enumerate(FONT_DATA):
        row_str = ", ".join(f"0x{b:02x}" if isinstance(b, int) else b for b in rows)
        ch = chr(i) if 0x20 <= i <= 0x7E else "."
        parts.append(f"\t/* 0x{i:02x} '{ch}' */ {{ {row_str} }},\n")
    parts.append("};\n")
...
def main(argv: list[str]) -> int:
    if len(argv) != 2:
        sys.stderr.write("usage: gen-font.py <out-header>\n")
        return 2

    out_path = Path(argv[1])
    emit_header(out_path)
    sys.stderr.write(f"gen-font.py: wrote {out_path} (source: Built-in VGA 8x16)\n")
    return 0
